...
class Multi_attention_Model(nn.Module):
    def __init__(self, opt, using_amp =False):
        ...
        # Correct model and tokenizer instantiation
        ...
        if opt.dataset == "AWA2" or opt.dataset == "CUB":
            self.deit = AutoModelForImageClassification.from_pretrained("facebook/deit-base-distilled-patch16-224")
        elif opt.dataset == "SUN":
            self.deit = SwinForImageClassification.from_pretrained("../swin_base_patch4_window7_224")
    ...
    def forward(self, x, attribute, texts, is_mask=False, contrast=False, mask_texts = [], mask_words = [], naive_contrast = False, whole_attribute=None, con_labels=[],mask_indexs = None, batch_target=None,attribute_deal=None,do_predict=False,texts_label=[],mask_for_predict_dict=[],impath=[],texts_label_withpro=[]):
        ...
        # print(f"inputs.attention_mask is {inputs.attention_mask}")
        # print(f"inputs.input_ids is {inputs.input_ids}")
        # print(f"inputs.token_type_ids is {inputs.token_type_ids}")
        ...
        #     input_ids=inputs.input_ids)
        # print(1)
        # text_embedding = self.bert(token_type_ids=inputs.token_type_ids)
        # print(2)
        # text_embedding = self.bert(attention_mask = inputs.attention_mask)

        text_hidden_state = text_embedding[0] # [24, 100, 768]
        # print(f"text_hidden_state shape {text_hidden_state}")
